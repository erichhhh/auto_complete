module AutoCompleteMacrosHelper      
  ...
  def text_field_with_auto_complete(object, method, tag_options = {}, completion_options = {})
    tag_options[:id] = "#{object}_#{method}" if !tag_options[:id]
    
    (completion_options[:skip_style] ? "" : auto_complete_stylesheet) +
    text_field(object, method, tag_options) +
    content_tag("div", "", :id => "#{tag_options[:id]}_auto_complete", :class => "auto_complete") +
    auto_complete_field("#{tag_options[:id]}", { :url => { :action => "auto_complete_for_#{object}_#{method}" } }.update(completion_options))
  end
